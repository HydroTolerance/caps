...
$notificationSql = "INSERT INTO notifications (user_id, message, appointment_id, created_at, is_read) VALUES (?, ?, ?, ?, 0)";
$notificationStmt = mysqli_prepare($conn, $notificationSql);
mysqli_stmt_bind_param($notificationStmt, "ssss", $userId, $notificationMessage, $lastInsertedId, $currentDate);
...
<?php
  include "../../db_connect/config.php";
    if (isset($_POST['update'])) {
        ...
        $query = "UPDATE zp_appointment SET email = ?, date = ?, `time` = ?, apt_reason = ?, appointment_status = 'Rescheduled (Derma)' WHERE id = ?";
        $stmt = mysqli_prepare($conn, $query);
            ...
            { 
                ...
                render: function(data, type, row) {
                    ...
                    return '<select class="form-select custom-width text-nowrap" data-id="' + row.id + '" name="status" ' + (isCompleted ? 'disabled' : '') + ' onchange="updateStatus(' + row.id + ', this.value)">' +
                        ...
                        '<option ' + (row.appointment_status === 'Completed' ? 'selected' : '') + ' value="Completed" hidden>Completed</option>' +
                        '<option ' + (row.appointment_status === 'Acknowledged' ? 'selected' : '') + ' value="Acknowledged" hidden>Acknowledged</option>' +
                        '<option ' + (row.appointment_status === 'Rescheduled (Admin)' ? 'selected' : '') + ' value="Rescheduled (Admin)"hidden>Rescheduled</option>' +
                        '<option ' + (row.appointment_status === 'Rescheduled (Derma)' ? 'selected' : '') + ' value="Rescheduled (Derma)" >Rescheduled</option>' +
                        ...
                        '<option ' + (row.appointment_status === 'Did not show' ? 'selected' : '') + ' value="Did not show" hidden>Did not Show</option>' +
                        '</select>';
                }
            }
        ],
...
statusFilter.on('change', function () {
    ...
    dataTable.column(9).search(selectedStatus).draw();
});
